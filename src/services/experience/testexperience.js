...
testRouter.get("/:userId/experiences", async (req, res, next) => {
  try {
...
testRouter.post("/:userId/experiences", async (req, res, next) => {
  try {
...
testRouter.get("/:userId/experiences/CSV", async (req, res, next) => {
  try {
...
testRouter.get("/:userId/experiences/:expId", async (req, res, next) => {
  try {
    ...
    if (User) {
      ...
      if (experience) {
        ...
        next(
          createError(404, `Experience with id ${req.params.expId} not found`)
        );
      }
    } else {
...
testRouter.put("/:userId/experiences/:expId", async (req, res, next) => {
  try {
    // FIND USER
    const User = await UserModel.findById(req.params.userId);
    // IF USER FIND EXPERIENCE
    if (User) {
      const foundExperience = User.experiences.find(
        (e) => e._id.toString() === req.params.expId
      );
      if (foundExperience) {
        const updatedExperience = {
          ...foundExperience.toObject(),
          ...req.body,
        };
...
testRouter.delete("/:userId/experiences/:expId", async (req, res, next) => {
  try {
    const User = await UserModel.findById(req.params.userId);
